...
class DistanceMap: 
    def __init__(self, map_name): 
        ...
        self.distance_map = image_handler.image_to_array(config.image_and_image_data_prefix+map_name+config.distance_map_suffix)

    def world_coordinates_to_image(self, coordinates):
        x_image = coordinates[0] * self.image_data['decimal_multiplier'] - self.image_data['x_min']
        y_image = coordinates[1] * self.image_data['decimal_multiplier'] - self.image_data['y_min']
    ...
    def get_distance_from_worlds(self, coordinates):
        image_coordinates = self.world_coordinates_to_image(coordinates)
        distance = self.distance_map[image_coordinates[1], image_coordinates[0]]
